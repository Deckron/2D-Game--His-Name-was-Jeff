	...
	float       slop)
{
	...
	gf2d_Rectangle_copy(space->bounds, bounds);
...
{
	if (!body) { slog("no body detected"); return; }
	if (body->mass != 0)
	{
...
void gf2d_space_draw(Space *space, Vector2D offset)
{
	...
	r = gf2d_Rectangle_to_sdl_Rectangle(space->bounds);
...
void gf2d_body_adjust_collision_overlap(Body *a, float slop, Rectangle bounds)
{
	Rectangle r;
	if (!a)return;
...
void gf2d_body_adjust_collision_bounds_velocity(Body *a, float slop, Rectangle bounds, Vector2D *velocity)
{
	Rectangle r;
	if (!a)return;
	r = gf2d_shape_get_bounds(*a->shape);
...
Uint8 gf2d_body_check_bounds(Body *body, Rectangle bounds, Vector2D *poc, Vector2D *normal)
{
	Shape aS = { 0 }, bs = { 0 };
	if (!body)return 0;
	bs = gf2d_shape_from_Rectangle(bounds);

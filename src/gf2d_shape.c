...
Vector2D gf2d_edge_get_normal_for_Rectangle(Edge e, Rectangle r)
{
	Vector2D out = { 0 };
	Vector2D c = { 0 };
	...
	c = gf2d_Rectangle_get_center_point(r);

	if (gf2d_point_in_Rectangle(vector2d(e.x1, e.y1), r))
	{
		out.x = e.x1 - e.x2;
		out.y = e.y1 - e.y2;
	...
	if (gf2d_point_in_Rectangle(vector2d(e.x2, e.y2), r))
	{
		out.x = e.x2 - e.x1;
		out.y = e.y2 - e.y1;
		vector2d_normalize(&out);
...
Vector2D gf2d_Rectangle_get_normal_for_edge(Rectangle r, Edge e)
{
	Vector2D out = { 0 };
	Vector2D c = { 0 };
	Vector2D n1, n2, p1, p2;
...
Vector2D gf2d_circle_get_normal_for_Rectangle(Circle c, Rectangle r)
{
	Vector2D out = { 0 };
	Vector2D poc = { 0 };
	if (!gf2d_circle_Rectangle_overlap_poc(c, r, &poc, NULL))
	{
...
Vector2D gf2d_Rectangle_get_normal_for_Rectangle(Rectangle r, Rectangle ref)
{
	Vector2D out = { 0 };
	if (ref.x + 1 >= (r.x + r.w))
	{
...
Vector2D gf2d_Rectangle_get_normal_for_cirlce(Rectangle r, Circle c)
{
	Vector2D out = { 0 };
	if (c.x < r.x)out.x = -1;
	if (c.y < r.y)out.y = -1;
	...
	if (gf2d_point_in_Rectangle(vector2d(c.x, c.y), r))
	{
		out = gf2d_Rectangle_get_center_point(r);
...
Vector2D gf2d_shape_get_normal_for_edge(Shape s, Edge e)
{
	...
	{
	case ST_RECT:
		out = gf2d_edge_get_normal_for_Rectangle(e, s.s.r);
		break;
	case ST_CIRCLE:
...
Vector2D gf2d_shape_get_normal_for_cirlce(Shape s, Circle c)
{
	...
	{
	case ST_RECT:
		out = gf2d_Rectangle_get_normal_for_cirlce(s.s.r, c);
		break;
	case ST_CIRCLE:
...
Vector2D gf2d_shape_get_normal_for_Rectangle(Shape s, Rectangle r)
{
	Vector2D out = { 0 };
	switch (s.type)
	{
	case ST_RECT:
		out = gf2d_Rectangle_get_normal_for_Rectangle(s.s.r, r);
		break;
	case ST_CIRCLE:
		out = gf2d_circle_get_normal_for_Rectangle(s.s.c, r);
		break;
	case ST_EDGE:
		out = gf2d_edge_get_normal_for_Rectangle(s.s.e, r);
		break;
	}
	return out;
}
...
Vector2D gf2d_shape_get_normal_for_shape(Shape s, Shape s2)
{
	...
	{
	case ST_RECT:
		out = gf2d_shape_get_normal_for_Rectangle(s, s2.s.r);
		break;
	case ST_CIRCLE:
...
{
	Rectangle r;
	gf2d_Rectangle_set(r, x, y, w, h);
	return r;
}

void gf2d_Rectangle_draw(Rectangle r, Color color)
{
	gf2d_draw_rect(gf2d_Rectangle_to_sdl_Rectangle(r), gf2d_Color_to_vector4(color));
}
...
void gf2d_shape_draw(Shape shape, Color color, Vector2D offset)
{
	...
	case ST_RECT:
		...
		gf2d_Rectangle_draw(shape.s.r, color);
		break;
	case ST_CIRCLE:
...
{
	return gf2d_Rectangle_overlap_poc(a, b, NULL, NULL);
}
...
{
	Rectangle newRectangle1, newRectangle2;
	gf2d_Rectangle_set(newRectangle1, b.x - a.r, b.y, b.w + a.r + a.r, b.h);
	gf2d_Rectangle_set(newRectangle2, b.x, b.y - a.r, b.w, b.h + a.r + a.r);
	...
	if ((gf2d_point_in_Rectangle(vector2d(a.x, a.y), newRectangle1)) ||
		(gf2d_point_in_Rectangle(vector2d(a.x, a.y), newRectangle2)))
	{
		if (poc)
		{
			if (a.x < b.x)
...
Uint8 gf2d_circle_Rectangle_overlap(Circle a, Rectangle b)
{
	...
	gf2d_Rectangle_set(newRectangle, b.x - a.r, b.y, b.w + a.r + a.r, b.h);
	if (gf2d_point_in_Rectangle(vector2d(a.x, a.y), newRectangle))return 1;
	gf2d_Rectangle_set(newRectangle, b.x, b.y - a.r, b.w, b.h + a.r + a.r);
	if (gf2d_point_in_Rectangle(vector2d(a.x, a.y), newRectangle))return 1;
...
Uint8 gf2d_shape_overlap_poc(Shape a, Shape b, Vector2D *poc, Vector2D *normal)
{
	...
	case ST_CIRCLE:
		...
		case ST_RECT:
			return gf2d_circle_Rectangle_overlap_poc(a.s.c, b.s.r, poc, normal);
		case ST_EDGE:
	...
	case ST_RECT:
		...
		{
		case ST_RECT:
			return gf2d_Rectangle_overlap_poc(a.s.r, b.s.r, poc, normal);
		case ST_CIRCLE:
			return gf2d_circle_Rectangle_overlap_poc(b.s.c, a.s.r, poc, normal);
		case ST_EDGE:
			return gf2d_edge_Rectangle_intersection_poc(b.s.e, a.s.r, poc, normal);
		}
	case ST_EDGE:
		switch (b.type)
		{
		...
		case ST_RECT:
			return gf2d_edge_Rectangle_intersection_poc(a.s.e, b.s.r, poc, normal);
		}
	}
	return 0;
}
...
Shape gf2d_shape_sdl_Rectangle(SDL_Rect r)
{
	Shape shape;
	shape.type = ST_RECT;
	shape.s.r.x = r.x;
...
Uint8 gf2d_Rectangle_to_intersection_poc(Edge e, Rectangle r, Vector2D *poc, Vector2D *normal)
{
	Uint8 ret;
	Uint8 i;
...
Uint8 gf2d_edge_Rectangle_intersection_poc(Edge e, Rectangle r, Vector2D *poc, Vector2D *normal)
{
	...
	{/*
		...
		{
			center = gf2d_Rectangle_get_center_point(r);
...
{
	return gf2d_edge_Rectangle_intersection_poc(e, r, NULL, NULL);
}
...
{
	slog("Rectangle: (%f,%f,%f,%f)", r.x, r.y, r.w, r.h);
}
...
Rectangle gf2d_edge_get_bounds(Edge e)
{
	Rectangle r;
	r.x = MIN(e.x1, e.x2);
	r.y = MIN(e.y1, e.y2);
...
Rectangle gf2d_circle_get_bounds(Circle c)
{
	Rectangle r;
	r.x = c.x - c.r;
	r.y = c.y - c.r;
